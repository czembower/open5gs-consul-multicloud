...
data "external" "pubkey_conversion" {
  program = [
    ...
    "--arg",
    "pubkey",
    "\"$(echo ${chomp(data.tls_certificate.eks_ca.certificates[0].cert_pem)} | base64 -d | openssl x509 -noout -pubkey | awk '{printf \"%s\n\", $0}')\"",
...
//jq -n --arg pubkey "$(echo asdf | base64 -d | openssl x509 -noout -pubkey | awk '{printf "%s\n", $0}')" '{"public_key_pem":$pubkey}'
